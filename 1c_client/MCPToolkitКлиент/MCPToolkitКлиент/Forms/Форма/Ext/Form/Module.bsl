...
Функция ПолучитьСписокМетаданныхРасширения(Расширение, ТипМетаданных, МаскаИмени, Лимит, Смещение, КэшПараметры = Неопределено)
	...
	Попытка
		...
		Для Каждого СтрИндекса Из Индекс Цикл
			...
			Если ВсегоПодходящих >= Смещение Тогда
				Если Возвращено < Лимит Тогда
		...
		// Ниже оставлен legacy-код (не выполняется из-за Возврат выше).
		...
		// Собираем объекты из расширения
		МаскаНижний = НРег(МаскаИмени);

		Для Каждого ОписаниеТипа Из ТипыДляОбработки Цикл
			Попытка
				КоллекцияВРасширении = ПолучитьКоллекциюИзРасширения(Расширение, ОписаниеТипа.Коллекция);
				Если КоллекцияВРасширении = Неопределено Тогда
					Продолжить;
				КонецЕсли;

				Для Каждого ОбъектМД Из КоллекцияВРасширении Цикл
					ПолноеИмя = ОписаниеТипа.Тип + "." + ОбъектМД.Имя;
					СинонимОбъекта = ОбъектМД.Синоним;

					// Фильтрация по маске
					Если НЕ ПустаяСтрока(МаскаИмени) Тогда
						Если Найти(НРег(ОбъектМД.Имя), МаскаНижний) = 0
							И Найти(НРег(СинонимОбъекта), МаскаНижний) = 0 Тогда
							Продолжить;
						КонецЕсли;
					КонецЕсли;

					ОбъектыМетаданных.Добавить(Новый Структура("ПолноеИмя, Синоним", ПолноеИмя, СинонимОбъекта));
				КонецЦикла;
			Исключение
				// Коллекция может быть недоступна в расширении
			КонецПопытки;
		КонецЦикла;

		// Сортировка по ПолноеИмя
		Для й = 0 По ОбъектыМетаданных.ВГраница() - 1 Цикл
			Для к = й + 1 По ОбъектыМетаданных.ВГраница() Цикл
				Если ОбъектыМетаданных[й].ПолноеИмя > ОбъектыМетаданных[к].ПолноеИмя Тогда
					Врем = ОбъектыМетаданных[й];
					ОбъектыМетаданных[й] = ОбъектыМетаданных[к];
					ОбъектыМетаданных[к] = Врем;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;

		// Пагинация
		ВсегоОбъектов = ОбъектыМетаданных.Количество();
		РезультатСписок = Новый Массив;

		Если Смещение < ВсегоОбъектов Тогда
			КонечныйИндекс = Мин(Смещение + Лимит - 1, ВсегоОбъектов - 1);
			Для Индекс = Смещение По КонечныйИндекс Цикл
				РезультатСписок.Добавить(ОбъектыМетаданных[Индекс]);
			КонецЦикла;
		КонецЕсли;

		Возвращено = РезультатСписок.Количество();
		Обрезано = (Смещение + Возвращено) < ВсегоОбъектов;
		СледующееСмещение = Смещение + Возвращено;
...
Функция ПолучитьДетальныеМетаданныеРасширения(Расширение, Фильтр, Секции, КэшПараметры = Неопределено)
	...
	Попытка
		// Парсим Фильтр: "ТипМетаданных.ИмяОбъекта"
		...
		ИнформацияОбОбъекте.Вставить("Тип", ТипОбъекта);
		...
		Исключение
			// У некоторых объектов нет реквизитов
		КонецПопытки;
		...
		Если СекцияЗапрошена(Секции, "forms") Тогда
			Попытка
				ИнформацияОбОбъекте.Вставить("forms", СекцияИзКоллекцииПолноеИмяСиноним(ОбъектМД.Формы));
			Исключение
			КонецПопытки;
		КонецЕсли;
		...
		Если СекцияЗапрошена(Секции, "commands") Тогда
			Попытка
				ИнформацияОбОбъекте.Вставить("commands", СекцияИзКоллекцииПолноеИмяСиноним(ОбъектМД.Команды));
			Исключение
			КонецПопытки;
		КонецЕсли;
		...
		Если СекцияЗапрошена(Секции, "layouts") Тогда
			Попытка
				ИнформацияОбОбъекте.Вставить("layouts", СекцияИзКоллекцииПолноеИмяСиноним(ОбъектМД.Макеты));
			Исключение
			КонецПопытки;
		КонецЕсли;
...
Функция ПолучитьИнформациюОКонфигурации()
	...
	Кандидаты.Добавить("Многоязычность");
...
Функция ПолучитьСписокМетаданных(ТипМетаданных = "", МаскаИмени = "", Лимит = 100, Смещение = 0)
	...
	Возврат Новый Структура("success, data, truncated, limit, returned, offset, has_more, next_offset",
		Истина,
		СписокОбъектов,
		ЕстьЕще,
		Лимит,
...
Функция ПолучитьДетальныеМетаданные(ПолноеИмя, Секции = Неопределено)
	...
	Попытка
		...
		КонецЦикла;
	Исключение
		// У некоторых объектов нет реквизитов (секция не применима)
		Реквизиты = Неопределено;
	КонецПопытки;
	...
	Если СекцияЗапрошена(Секции, "layouts") Тогда
		...
		Если КоллекцияМакетов <> Неопределено Тогда
			ИнформацияОбОбъекте.Вставить("layouts", СекцияИзКоллекцииПолноеИмяСиноним(КоллекцияМакетов));
		КонецЕсли;
	КонецЕсли;
