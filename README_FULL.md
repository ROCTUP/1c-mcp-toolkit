...
#### Автоматический запуск обработки из командной строки

Обработку можно открыть из командной строки 1С и сразу выполнить нужный режим подключения через параметр `/C "startup;..."`. Этот механизм относится ко всем вариантам запуска: он может запустить встроенный сервер внутри обработки или подключить обработку к уже запущенному Python/Docker-прокси.
...
1cv8.exe ENTERPRISE `
  ...
  /N "<пользователь-1С>" `
  /P "<пароль-1С>" `
  /Execute "<путь-к-MCP_Toolkit.epf>" `
  /C "startup;<параметры>"
```
...
```powershell
1cv8.exe ENTERPRISE `
  /F "G:\1C\1C_DB\Документооборот_3.0_Демо" `
  /N "Администратор" `
  /Execute "D:\MCP_Toolkit.epf" `
  /C "startup;mode=embedded;port=6003"
```

Подключение обработки к прокси:
...
Запуск с парольной аутентификацией:

```powershell
1cv8.exe ENTERPRISE `
  /F "G:\1C\1C_DB\Документооборот_3.0_Демо" `
  /N "Белугин М.А. (руководитель отдела производства)" `
  /P "12345678" `
  /Execute "D:\MCP_Toolkit.epf" `
  /C "startup;mode=embedded;port=6004;auth=password;pwd=12345678"
```

В этом варианте `/P` используется платформой 1С для входа в базу, а `pwd=...` передаётся обработке и хранится только в памяти формы. Это нужно, чтобы последующий `restart_1c_session` мог запустить новую сессию с парольной аутентификацией.

Запуск с Windows-аутентификацией:
...
1cv8.exe ENTERPRISE `
  ...
  /WA `
  /Execute "D:\MCP_Toolkit.epf" `
  /C "startup;mode=embedded;port=6006;auth=windows"
```

Для Windows-аутентификации используйте `/WA` без `/N` и `/P`: платформа 1С выбирает пользователя по текущей Windows-учётной записи. Внутри обработки `auth=windows` настраивает Windows-аутентификацию для последующих перезапусков сессии.

Параметры `startup`:

| Параметр | Значение |
|---|---|
| `mode=embedded` | Запустить встроенный HTTP/MCP-сервер |
| `mode=proxy` | Подключиться к Python/Docker-прокси |
| `port=6003` | Порт встроенного сервера |
| `url=http://...` | Адрес прокси-сервера |
| `channel=...` | Идентификатор канала для прокси |
| `auth=windows` | Использовать Windows-аутентификацию для последующего `restart_1c_session` |
| `auth=password` | Использовать парольную аутентификацию для последующего `restart_1c_session` |
| `pwd=...` | Пароль текущего пользователя 1С для последующего перезапуска; не сохраняется в хранилище |

Неуказанные параметры берутся из сохранённых настроек обработки. Если сохранённых настроек нет, используются значения по умолчанию: встроенный сервер и порт `6003`.

Если значение параметра содержит символы `;`, `=` или другие специальные символы командной строки, передавайте его в URL-encoded виде. Значения параметров декодируются обработкой при разборе `startup`.
...
> (поля `command`/`args`). Конфигурация с `url`/`transport` (как для Kiro IDE выше)
> Claude Desktop **не понимает** — запись игнорируется. Используйте мост
> `mcp-remote` или Custom Connector.
...
> Используйте `127.0.0.1`, а не `localhost` (надёжнее с IPv4/IPv6-биндингом).
> Флаг `--allow-http` разрешает подключение по http без TLS.
...
AnythingLLM подключается напрямую к запущенному HTTP-серверу. Добавьте сервер в
конфигурацию MCP (`anythingllm_mcp_servers.json` либо через UI):
...
> Тип транспорта указывается как `type: "streamable"` — это обязательно.
> Если поле не задать, AnythingLLM считает сервер `sse` и не подключится.
...
> Работает **только в режиме «Встроенный сервер»**. В режиме прокси роль аутентификации выполняет `channel id`.

По умолчанию встроенный сервер принимает запросы без аутентификации. Чтобы её включить:

1. На форме обработки в блоке настроек встроенного сервера в поле **«Токен доступа»** укажите токен либо нажмите **«Сгенерировать»** (создаётся случайный UUID).
2. Токен сохраняется автоматически. Пустое поле = аутентификация выключена (поведение по умолчанию, обратная совместимость).
3. Пропишите тот же токен в конфигурации MCP-клиента как HTTP-заголовок `Authorization: Bearer <token>`.

Если токен задан, все входящие запросы к `/mcp` (и `/api/*`) без корректного заголовка получают `401 Unauthorized`. Эндпоинт `/health` остаётся открытым.

**Kiro IDE** — добавьте поле `headers`:
...
**AnythingLLM** — сохраните обязательное `"type": "streamable"` и добавьте `headers`:
...
> Не убирайте `"type": "streamable"` — без него AnythingLLM считает сервер `sse` и не подключается (см. раздел [AnythingLLM](#anythingllm) выше).

**Claude Desktop (мост `mcp-remote`)** — передайте заголовок аргументом `--header`:
...
> **Границы безопасности.** Токен авторизует операции 1С, но не является сетевым периметром и не защищает от DoS: проверка выполняется уже после того, как встроенный сервер принял соединение. Защиту сетевого доступа обеспечивают адрес привязки (bind) и внешний контур (firewall / reverse proxy / VPN). Не выставляйте порт встроенного сервера в недоверенную сеть без внешней защиты.

> **Ротация токена.** Смена токена применяется к новым запросам сразу, но уже открытые SSE-соединения не разрываются. Для немедленного отзыва доступа перезапустите встроенный сервер (остановить → запустить) — это закроет все потоки.

> **Legacy SSE.** Для legacy-транспорта аутентификация проверяется при установке потока (`GET /mcp`) — неверный токен возвращает `401` и поток не открывается. Отдельный `POST /mcp/message` из-за fire-and-forget контракта компоненты всегда получает транспортный `202`, но неавторизованное сообщение при этом отбрасывается и не обрабатывается.

### MCP-инструменты

Прокси предоставляет MCP-инструменты для взаимодействия с 1С:Предприятие (количество зависит от режима и ОС):
...
| `execution_context` | string | Нет | Контекст выполнения: `"server"` (по умолчанию) — `&НаСервереБезКонтекста`, доступ к БД и объектам 1С; `"client"` — `&НаКлиенте`, доступ к реквизитам формы и UI-функциям, без запросов к БД |

**Примеры:**

```json
// Получить текущую дату (серверный контекст по умолчанию)
{
  "name": "execute_code",
  "arguments": {
...
// Клиентский контекст — работа с формой и UI
{
  "name": "execute_code",
  "arguments": {
    "code": "ОткрытьФорму(\"Справочник.Номенклатура.ФормаСписка\"); Результат = \"OK\";",
...
| `sections` | array[string] | Нет | Секции детального ответа (работает только с `filter`; **несовместим с `attribute_mask`**): `properties`, `forms`, `commands`, `layouts`, `predefined`, `movements`, `characteristics`. Примечание: `movements` применима только к объектам типа `Документ` — для остальных типов игнорируется |
...
| `attribute_mask` | string | Нет | Поиск по имени/синониму атрибута (реквизита, измерения, ресурса, реквизита ТЧ) — регистронезависимая подстрока. Возвращает тот же формат что режим списка: `data=[{ПолноеИмя, Синоним},...]`. Совместим с `meta_type`, `name_mask`, `filter` (только корневой объект), `extension_name` (конкретное расширение). **Несовместим с `sections`** (вернёт ошибку). |
...
// Сегменты пути — в единственном числе: Реквизит, Измерение, Ресурс, ТабличнаяЧасть, РеквизитАдресации, СтандартныйРеквизит
{
  "name": "get_metadata",
  "arguments": {
...
// Найти все атрибуты с "контраг" в имени или синониме (режим 5)
{
  "name": "get_metadata",
  "arguments": {
    "attribute_mask": "контраг"
  }
}

// Поиск атрибутов внутри одного объекта
{
  "name": "get_metadata",
  "arguments": {
    "attribute_mask": "дата",
    "filter": "Документ.Реализация"
  }
}

// Round-trip: найти атрибут → получить его детали
// Шаг 1: найти
{
  "name": "get_metadata",
  "arguments": {
...
// Шаг 2: data[0]["ПолноеИмя"] → передать в filter
{
  "name": "get_metadata",
  "arguments": {
    "filter": "Документ.Реализация.Реквизит.Контрагент",
    "sections": ["properties"]
  }
}
```
...
**Ответ (режим поиска атрибутов, с attribute_mask):**
```json
{
  "success": true,
  "truncated": false,
...
Поиск по встроенному справочнику языка BSL: функции, методы, типы, языковые конструкции. Требует загруженную компоненту SyntaxHelpReader на стороне 1C (доступна в обоих режимах — встроенный сервер и прокси). Расположение файлов справки определяется автоматически — используются стандартные файлы `.hbk`, поставляемые в составе платформы 1С:Предприятие.

Поиск возвращает список *кандидатов* — уникальных путей-хлебных крошек. Когда найден ровно один кандидат с содержимым, возвращается Markdown-справка по нему. Если найденный кандидат является навигационным разделом (нет содержимого, есть подтемы), вместо него возвращаются прямые подтемы в виде кандидатов — можно продолжать навигацию. Если кандидатов несколько, нужно уточнить запрос или использовать путь кандидата как точное ключевое слово.

**Параметры:**

| Параметр | Тип | Обязательный | По умолчанию | Описание |
|----------|-----|--------------|--------------|----------|
| `keywords` | array of string | Да | — | Ключевые слова для поиска или точный путь кандидата / ссылка из контента |
| `match` | string | Нет | `"all"` | Режим совпадения: `"all"` — все слова должны совпасть, `"any"` — любое |
| `limit` | integer | Нет | 100 | Максимальное число кандидатов в ответе (1–300) |
| `offset` | integer | Нет | 0 | Смещение для постраничного перебора кандидатов |
| `content_page` | integer | Нет | 1 | Номер страницы контента (1-based). Если `content_has_more: true` — вызвать с `content_page + 1` |

**Ответ (несколько кандидатов):**
```json
{
  "success": true,
  "data": {
...
**Ответ (один кандидат, контент помещается на одну страницу):**
```json
{
  "success": true,
  "data": {
...
**Ответ (контент разбит на страницы):**
```json
{
  "success": true,
  "data": {
...
Поля `content_page`, `content_total_pages`, `content_has_more` присутствуют **только когда `content` не null**.
...
// Поиск по ключевому слову
{
  ...
  "arguments": { "keywords": ["Найти"] }
}

// Уточнить поиск: методы конкретного типа
{
  "name": "get_bsl_syntax_help",
  "arguments": { "keywords": ["Найти", "Массив"] }
}

// Точный поиск по пути кандидата
{
  "name": "get_bsl_syntax_help",
  "arguments": { "keywords": ["Массив/Методы/Найти"] }
}

// Точный поиск по ссылке из контента (topic: передаётся как есть)
{
  "name": "get_bsl_syntax_help",
  "arguments": { "keywords": ["topic:Массив/Методы/Найти"] }
}

// Получить следующую страницу контента
{
  "name": "get_bsl_syntax_help",
  "arguments": { "keywords": ["Запрос"], "content_page": 2 }
}
```

#### 10. get_screenshot

Делает снимок активного окна 1С:Предприятие и возвращает его в виде PNG-изображения.

> **Доступность:** только на **Windows** (требует Native API компоненту ScreenCapture — Windows DLL). На Linux инструмент не появляется в списке `tools/list`. Доступен только через MCP; REST API эндпоинт отсутствует.

**Параметры:**
| Параметр | Тип | Обязательный | По умолчанию | Описание |
|----------|-----|--------------|--------------|----------|
| `form_name` | string | Нет | — | Полное имя формы 1С (например, `"Справочник.Контрагенты.Форма.ФормаЭлемента"`). Если указано, форма открывается перед снимком и автоматически закрывается после. Если не указано — снимается текущее активное окно. |
| `scale_percent` | integer | Нет | `100` | Масштаб выходного изображения (10–200). Значения >100 увеличивают (200 = 2× размер с большей детализацией), <100 уменьшают (50 = половина размера). Метки координат сетки всегда показывают исходные пиксели окна, независимо от масштаба. |
| `show_grid` | boolean | Нет | `false` | Если `true`, наносит сетку на снимок (до 10×10, динамическая плотность в зависимости от размера изображения — маленькие регионы могут получить меньше линий). Красные линии, жёлтые метки координат (px) на чёрном фоне на каждой оси, красные точки на пересечениях. Начало координат (0, 0) — верхний левый угол захваченного изображения. Координаты линий сетки также возвращаются в поле `grid_coords`. |
| `region` | object | Нет | null | Обрезка до под-региона перед масштабированием. Объект с целыми полями `x` (≥0), `y` (≥0), `width` (≥1), `height` (≥1). Координаты в пикселях клиентской области исходного окна. Отсутствие или `null` — полное окно. Возвращает ошибку если регион выходит за границы окна. |
| `highlight_rects` | array | Нет | null | Массив прямоугольников для рисования на снимке в виде нумерованных синих рамок (3 px, RGB 0,120,255). Каждый прямоугольник: `{x, y, width, height}` в пикселях клиентской области (то же пространство координат, что у меток сетки). Прямоугольники нумеруются с 1. Метки выводятся снаружи над правым верхним углом (внутри, если места сверху недостаточно). Прямоугольники не должны перекрываться — при перекрытии возвращается ошибка. Максимум 20 элементов. |

**Примеры:**

```json
// Снять текущее активное окно
{
  "name": "get_screenshot",
  "arguments": {}
}

// Открыть форму и снять её
{
  "name": "get_screenshot",
  "arguments": {
    "form_name": "Справочник.Номенклатура.Форма.ФормаСписка"
  }
}

// Снять с сеткой и масштабом 150%
{
  "name": "get_screenshot",
  "arguments": {
    "show_grid": true,
...
// Снять с подсветкой элементов (например, после определения координат по сетке)
{
  "name": "get_screenshot",
  "arguments": {
    "highlight_rects": [
...
Инструмент возвращает изображение PNG напрямую как `image` content (не обёрнутое в `data`). Дополнительно:
```json
{
  "success": true,
  "image_base64": "<base64-encoded PNG>",
  "mime_type": "image/png",
  "window_rect": {"left": 0, "top": 0, "width": 1920, "height": 1080}
}
```

| Поле | Описание |
|------|----------|
| `image_base64` | PNG-изображение в кодировке Base64 |
| `mime_type` | Всегда `"image/png"` |
| `window_rect` | Координаты и размеры захваченного окна (в пикселях экрана). Присутствует только если компонента вернула координаты (новые версии DLL). |
...
|----------|-------------|
| Компонента не загружена (Linux / ошибка загрузки DLL) | `ScreenCapture AddIn не загружен` |
| Форма не открылась | `Ошибка открытия формы: ...` / `Форма не открылась: <имя>` |
| Окно 1С не найдено | `Не удалось захватить окно (главное окно не найдено)` |
| Регион выходит за границы | `Указанный регион выходит за пределы клиентской области окна` |
| Прямоугольники перекрываются | `Rectangle N is outside the captured area` |

**Типичный сценарий использования (итеративный):**

1. Вызвать `get_screenshot` без параметров — получить снимок текущего состояния экрана
2. Вызвать `get_screenshot` с `show_grid: true` — увидеть координаты элементов
3. Вызвать `get_screenshot` с `highlight_rects` — подсветить интересующие элементы для пользователя
4. Вызвать `execute_code` с `execution_context: "client"` — выполнить действие с UI (например, открыть форму)
5. Повторить снимок для проверки результата

#### 11. submit_for_deanonymization

Отправляет финальный ответ агента для деанонимизации и отображения пользователю. **Доступен только при включённой анонимизации.**

> **Важно:** Этот инструмент НЕ возвращает деанонимизированный текст агенту. Он только подтверждает получение (`{"received": true}`). Реальные значения показываются исключительно пользователю в его интерфейсе 1С.

**Параметры:**
| Параметр | Тип | Обязательный | По умолчанию | Описание |
|----------|-----|--------------|--------------|----------|
| `text` | string | Да | - | Полный текст финального ответа, содержащий токены анонимизации |

**Когда вызывать:**
- **НЕОБХОДИМО** вызвать если, и только если, финальный ответ пользователю содержит токены анонимизации вида `[CATEGORY-NNNNN]` (например, `[ORG-00001]`, `[PER-00042]`, `[INN-00001]`)
- Вызвать **ровно один раз**, непосредственно перед отправкой финального ответа пользователю
- **НЕ** вызывать для промежуточных рассуждений, черновиков или сырых результатов инструментов
...
```json
{
  ...
  "arguments": {
    "text": "Компания [ORG-00001], ИНН [INN-00001], директор: [PER-00001]"
  }
}
```
...
**Важно:** После вызова этого инструмента отправляйте свой оригинальный финальный ответ с токенами пользователю — пользователь увидит деанонимизированную версию на своей стороне.

#### 12. restart_1c_session

Перезапускает текущую 1С-сессию — ту, которая выполняет вызовы инструментов. Обычно требуется после обновления конфигурации 1С (метаданных, расширений, BSL-кода), чтобы новая сессия подхватила изменения.

**Параметры:** отсутствуют.

**Предварительная настройка на форме обработки:**

В секции **«Параметры перезапуска сессии»** необходимо выбрать метод аутентификации, под которым запустится новая сессия:
- **Windows** — аутентификация ОС; доступна только на Windows
- **Пароль** — аутентификация по паролю; необходимо ввести пароль текущего пользователя ИБ в поле «Пароль перезапуска»

> Пароль не сохраняется в настройках и хранится только в памяти на время текущего сеанса работы с обработкой.

При парольной аутентификации также доступно поле **«Доп. ключ шифрования»** (до 20 символов). Этот ключ добавляется к базовому ключу шифрования при передаче пароля между сессиями, делая шифрование непредсказуемым для стороннего наблюдателя. Ключ генерируется автоматически при первом открытии формы и сохраняется в настройках. Можно изменить его вручную или сгенерировать новый кнопкой **«Сгенерировать»**. Поскольку ключ хранится в общих настройках, и старая, и новая сессия используют одно и то же значение — дополнительных действий не требуется.

> Не меняйте ключ шифрования во время выполнения перезапуска.

**Поведение:**
- Новая сессия запускается автоматически с теми же параметрами подключения к базе данных
- Состояние анонимизации сохраняется в новой сессии
- Текущая сессия завершается после того, как новая готова к работе
- Таймаут ожидания: не менее 150 секунд

> **Первый запуск:** при первом перезапуске платформа 1С может показать диалог подтверждения запуска внешней программы — это стандартный механизм безопасности 1С. Необходимо разрешить выполнение.
...
{
  "success": false,
  "error": "Restart or close already in progress"
}
```

#### 13. close_1c_session

Закрывает текущую 1С-сессию и возвращает команду скрипта-лаунчера для запуска новой. Новая сессия запускается с теми же параметрами подключения к базе данных; состояние анонимизации сохраняется. Используется когда необходим эксклюзивный доступ к базе данных (например, при обновлении конфигурации).

**Параметры:** отсутствуют.

**Предварительная настройка на форме обработки:**

В секции **«Параметры перезапуска сессии»** необходимо выбрать метод аутентификации, под которым скрипт-лаунчер запустит новую сессию:
- **Windows** — аутентификация ОС; доступна только на Windows
- **Пароль** — аутентификация по паролю; необходимо ввести пароль текущего пользователя ИБ в поле «Пароль перезапуска»

> Пароль не сохраняется в настройках и хранится только в памяти на время текущего сеанса работы с обработкой.

При парольной аутентификации также доступно поле **«Доп. ключ шифрования»** (до 20 символов). Этот ключ добавляется к базовому ключу шифрования при передаче пароля 1С в скрипт-лаунчер, делая шифрование непредсказуемым для стороннего наблюдателя. Ключ генерируется автоматически при первом открытии формы и сохраняется в настройках. Можно изменить его вручную или сгенерировать новый кнопкой **«Сгенерировать»**.

> Пароль для запуска лаунчера (`launch_password`) защищён отдельным одноразовым случайным ключом, который передаётся лаунчеру напрямую — пользовательский ключ шифрования на него не влияет и никуда не передаётся.

> **Первый запуск:** при первом запуске скрипта-лаунчера платформа 1С может показать диалог подтверждения запуска внешней программы — это стандартный механизм безопасности 1С. Необходимо разрешить выполнение.

**Порядок использования:**
1. Вызвать инструмент — текущая сессия закрывается, в поле `data` ответа возвращается текст с командой запуска скрипта-лаунчера и инструкцией
2. Выполнить полученную команду **синхронно** (команда уже содержит путь к скрипту с нужными параметрами):
   - **Windows:** `powershell -ExecutionPolicy Bypass -File '<path.ps1>'`
   - **Linux:** `bash '<path.sh>'`
3. Коды возврата:
   - **exit 0** — новая сессия готова к работе
   - **exit 1 (таймаут)** — состояние запуска неизвестно; проверьте, запустился ли процесс 1С, прежде чем запускать повторно
   - **exit 1 (не таймаут)** — читать вывод: ошибки до запуска (pre-launch) безопасно повторить после исправления причины; ошибки запуска — упавший новый экземпляр закрывается автоматически
4. На **Linux** с парольной аутентификацией — `python3` должен быть доступен в PATH
...
**Ответ при успехе** (поле `data` содержит команду запуска и инструкцию):
```
Session closed. To start a new session, run:
powershell -ExecutionPolicy Bypass -File '<path\to\launch.ps1>'
Run synchronously. Exit 0 = session ready. Non-zero = startup failed.
...
| `/api/get_bsl_syntax_help` | POST | Справочник языка BSL | Инструмент `get_bsl_syntax_help` |
| `/api/submit_for_deanonymization` | POST | Отправить текст для деанонимизации (только при включённой анонимизации) | Инструмент `submit_for_deanonymization` |
| `/api/restart_1c_session` | POST | Перезапустить сессию 1С | Инструмент `restart_1c_session` |
| `/api/close_1c_session` | POST | Закрыть сессию и получить команду запуска новой | Инструмент `close_1c_session` |

#### Формат ответов REST API

Большинство ответов REST API следуют единой структуре:
...
**Исключение:** Эндпоинт `submit_for_deanonymization` возвращает `{"received": true}` при успехе (без поля `data`).
...
| `/api/submit_for_deanonymization` | POST | REST: Отправить текст для деанонимизации (только при включённой анонимизации) | ✅ Да (`?channel=<id>`) |
| `/api/restart_1c_session` | POST | REST: Перезапустить сессию 1С | ✅ Да (`?channel=<id>`) |
| `/api/close_1c_session` | POST | REST: Закрыть сессию и получить команду запуска новой | ✅ Да (`?channel=<id>`) |
...
| Токен доступа | (пусто) | Если задан — входящие запросы должны присылать `Authorization: Bearer <token>`, иначе `401`. Пусто = аутентификация выключена. Кнопка «Сгенерировать» создаёт случайный токен (UUID). См. [Аутентификация по токену](#аутентификация-по-токену-bearer) |
...
#### Управление активными MCP-инструментами

В режиме «Встроенный сервер» можно включать и выключать отдельные MCP-инструменты прямо с формы, не перезапуская сервер.

В секции **«MCP инструменты»** расположены тумблеры для каждого из инструментов. При переключении тумблера кэш списка инструментов сбрасывается, и следующий запрос `tools/list` от MCP-клиента вернёт актуальный набор. Настройки сохраняются между сеансами автоматически.

> **Примечание:** тумблер `get_screenshot` отключает инструмент независимо от ОС. Но даже при включённом тумблере инструмент появится в `tools/list` только если компонента ScreenCapture успешно загрузилась (только Windows).

> Если MCP-клиент закэшировал список инструментов, необходимо переподключиться (reconnect), чтобы он увидел изменения.
